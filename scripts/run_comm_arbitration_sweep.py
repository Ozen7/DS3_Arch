#!/usr/bin/env python3
"""
Automated Experiment Runner for DS3 (DASH-Sim) - COMM Arbitration Type Sweep

This script automates running experiments across multiple:
- COMM SoC configurations (auto-generated via create_COMM_SoC.py)
- COMM Workload configurations (WIFI RX/TX job mixes)
...
Usage:
    python3 run_comm_arbitration_sweep.py
"""
...
# Fixed bandwidth for COMM arbitration sweep (bytes/microsecond)
bandwidth = 5280

# SoC configurations to generate and test
# Format: (filename, pe_specs)
# pe_specs: dict of {pe_type: count}
# Note: a7 and mm stay at 1 (required but not swept)
SOC_CONFIGS = [
    # Baseline scaled configurations
    ('SoC.COMM_a1_A1_m1_f1_v1.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 1, 'vit': 1}),
    ('SoC.COMM_a1_A2_m1_f2_v2.txt',
     {'a7': 1, 'a15': 2, 'mm': 1, 'fft': 2, 'vit': 2}),
    ('SoC.COMM_a1_A3_m1_f3_v3.txt',
     {'a7': 1, 'a15': 3, 'mm': 1, 'fft': 3, 'vit': 3}),

    # Individual A15 scaling
    ('SoC.COMM_a1_A4_m1_f1_v1.txt',
     {'a7': 1, 'a15': 4, 'mm': 1, 'fft': 1, 'vit': 1}),
    ('SoC.COMM_a1_A5_m1_f1_v1.txt',
     {'a7': 1, 'a15': 5, 'mm': 1, 'fft': 1, 'vit': 1}),
    ('SoC.COMM_a1_A6_m1_f1_v1.txt',
     {'a7': 1, 'a15': 6, 'mm': 1, 'fft': 1, 'vit': 1}),

    # Individual FFT scaling
    ('SoC.COMM_a1_A1_m1_f4_v1.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 4, 'vit': 1}),
    ('SoC.COMM_a1_A1_m1_f5_v1.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 5, 'vit': 1}),
    ('SoC.COMM_a1_A1_m1_f6_v1.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 6, 'vit': 1}),

    # Individual Viterbi scaling
    ('SoC.COMM_a1_A1_m1_f1_v4.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 1, 'vit': 4}),
    ('SoC.COMM_a1_A1_m1_f1_v5.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 1, 'vit': 5}),
    ('SoC.COMM_a1_A1_m1_f1_v6.txt',
     {'a7': 1, 'a15': 1, 'mm': 1, 'fft': 1, 'vit': 6}),
]

# Schedulers to test (same as RELIEF)
SCHEDULERS = [
    'RELIEF',
    'LL',
    'GEDF_D',
...
# Workload configurations for COMM (WIFI RX/TX)
# job_file order: job_WIFI_5RXM.txt (RX), job_WIFI_5TXM.txt (TX)
# job_list format: [RX_count, TX_count]
WORKLOAD_CONFIGS = [
    {'name': 'single_tx', 'job_list': '[[0,1]]', 'job_probabilities': '[]'},
    {'name': 'single_rx', 'job_list': '[[1,0]]', 'job_probabilities': '[]'},
    {'name': 'mixed_1rx_1tx', 'job_list': '[[1,1]]', 'job_probabilities': '[]'},
    {'name': 'mixed_2rx_1tx', 'job_list': '[[2,1]]', 'job_probabilities': '[]'},
    {'name': 'mixed_1rx_2tx', 'job_list': '[[1,2]]', 'job_probabilities': '[]'},
    {'name': 'mixed_2rx_2tx', 'job_list': '[[2,2]]', 'job_probabilities': '[]'},
    {'name': 'mixed_3rx_3tx', 'job_list': '[[3,3]]', 'job_probabilities': '[]'},
]
...
CREATE_SOC_SCRIPT = os.path.join(SCRIPT_DIR, 'create_COMM_SoC.py')
...
def generate_soc_config(soc_filename, pe_specs, bandwidth):
    """
    Generate SoC configuration file using create_COMM_SoC.py.

    Args:
        soc_filename: Name of the SoC configuration file
        pe_specs: Dictionary of {pe_type: count}
        bandwidth: Memory bandwidth in bytes/microsecond (not used - hardcoded in create_COMM_SoC.py)
    ...
    # Build command-line arguments for create_COMM_SoC.py
    # Format: python3 create_COMM_SoC.py a7 1 a15 1 mm 1 fft 1 vit 1
    cmd_args = ['python3', CREATE_SOC_SCRIPT]

    # Add PE type and count pairs in the correct order
    # Order from create_COMM_SoC.py: a7, a15, mm, fft, vit
    pe_order = ['a7', 'a15', 'mm', 'fft', 'vit']

    for pe_type in pe_order:
        if pe_type in pe_specs and pe_specs[pe_type] > 0:
            cmd_args.extend([pe_type, str(pe_specs[pe_type])])

    # Note: create_COMM_SoC.py has hardcoded bandwidth of 5280, no environment variable needed
...
def modify_config(soc_config, scheduler, workload_config, arbitration_type):
    ...
    config['DEFAULT']['job_file'] = 'job_WIFI_5RXM.txt,job_WIFI_5TXM.txt'
...
def run_experiments():
    ...
    """
    print("="*70)
    print("DS3 (DASH-Sim) Automated Experiment Runner - COMM Arbitration Type Sweep")
    print("="*70)
    ...
    for soc_filename, pe_specs in SOC_CONFIGS:
        success = generate_soc_config(soc_filename, pe_specs, bandwidth)
        if not success:
    ...
    for arbitration_type in ARBITRATION_TYPES:
        ...
        OUTPUT_CSV = os.path.join(RESULTS_DIR, f'experiment_results_COMM_NoCrit_{arbitration_type}_5280.csv')
    ...
    for arb_type in ARBITRATION_TYPES:
        csv_path = os.path.join(RESULTS_DIR, f'experiment_results_COMM_NoCrit_{arb_type}_5280.csv')
        print(f"  - {csv_path}")
